...
class TrainData(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        garbage1, comp1 = pad(torch.zeros(1, EMBEDDING_DIM, 25), main_title)
        garbage2, comp2 = pad(torch.zeros(1, EMBEDDING_DIM, 100), main_body)
        comp1 = [1.0 / comp1[0]]
        comp2 = [1.0 / comp2[0]]
        for elt in self.Xmatrix[index, 1:]:
            ...
            garbo1, comp3 = pad(torch.zeros(1, EMBEDDING_DIM, 25), query_title)
            garbo2, comp4 = pad(torch.zeros(1, EMBEDDING_DIM, 100), query_body)
        ...
        x = torch.cat((garbage1, garbage2), 2)
        return {
            "x": x,
            "pad_title": torch.from_numpy(comp1),
